const X_DESCRIPTION_FALLBACKS = [
  'see this post on x',
  'see this post on twitter',
  'join x to see',
];

const X_TEXT_KEYS = ['articleBody', 'caption', 'description', 'headline', 'text', 'name'] as const;
...
function cleanXTitle(title: string | null): string | null {
...
function isUsefulDescription(description: string): boolean {
  ...
  return !X_DESCRIPTION_FALLBACKS.some((phrase) => lowered === phrase || lowered.startsWith(`${phrase} `));
}
...
function extractStructuredXText(html: string): string | null {
...
export function extractXMetadata(html: string): { title: string | null; text: string | null } {
  ...
  const structuredText = extractStructuredXText(html);
...
export async function fetchXMetadata(url: string): Promise<{ title: string | null; text: string | null }> {
  ...
  if (!response.ok) {
    throw new Error(`X fetch failed with status ${response.status}`);
  }

  const html = await response.text();
  return extractXMetadata(html);
}
